...
from data.loader import NPZData, NPZData_single_view
import argparse
...
if __name__ == '__main__':
    ...
    # 2. init model
    m = UNet(in_channels=1, out_channels=1, last_activation='sigmoid').cuda()
    ...
    # 3. init dataloader
    train_data = NPZData_single_view(train_data_dir, downsample=2, pad=12, binarize=True, noise=True)  # convert data to (512, 512)
    test_data = NPZData_single_view(test_data_dir, downsample=2, pad=12, binarize=True, noise=True)
    ...
    for e in range(epochs):
        ...
        if test_loss < min_test_los:
            torch.save(m.state_dict(), checkpoint_dir / f"model_{e}_val_dice_{test_loss:.4f}.pth")
